...
linesa = g.readlines()
chain = []
numchain = int((len(linesa)-2)*density//nAB)
print "number of chains is %d"%numchain

#print (len(linesa)-2)*density//nAB*nAB
#select chains according to density
for i in range(0,int((len(linesa)-2)*density//nAB*nAB)):
    chain.append(np.array(linesa[i+2].split(),dtype = float))
        
chain = np.asarray(chain)
#separate two different kinds of beads
m = np.arange(0, len(chain), 1)
mask = m%nAB<nA
t0 = np.asarray(chain[mask,1:])
t1 = np.asarray(chain[-mask,1:])
...
for i in range(1,len(t1)):
    if(i%nB != 0):
        bond += lenp(t1[i-1],t1[i])
        k += 1
    else:
        continue
bond /= k

print "bondlength for coarse grain bead is %f  " % bond
...
print "bondlength for scaled coarse grain bead is %f  " % bond
...
for i in range(num_chains):
    ...
    for k in range(nA):
        if k == (nA-1):
            mpb = int(lenp(t1[i*nB],t0[(i+1)*nA-1])/bondm)
        else:
        ...
        ##Storing improper information of monomer
                ...
                if (j==0):
                    if(k == nA-1):
                        vec = t1[nB*i]-t0[(i+1)*nA-1]
                    else:   
                        vec = t0[i*nA+k+1]-t0[i*nA+k]
                    lentv = lent(vec)
                    #the vector which is the direction of two bead
                    vec = vec/lent(vec)
                    #a unit vector which is vertical t vec
    ...
    for k in range(nB):
        if(k == nB-1):
            mpb = int(lenp(t1[i*nB+k],t1[i*nB+k-1])/bondm)
        else:
            mpb = int(lenp(t1[i*nB+k+1],t1[i*nB+k])/bondm)
        totm += mpb
        for j in range(mpb):
            if j!=mpb-1 or k!=nB-1:
                #internal monomer
                if(j==0):
                    if(k == nB-1):
                        vec = np.asarray(t1[i*nB+k]-t1[i*nB+k-1])
                    else:
                        vec = np.asarray(t1[i*nB+k+1]-t1[i*nB+k])
                    lentv = lent(vec)
                    #the vector which is the direction of two bead
                    vec = vec/lent(vec)
                    #a unit vector which is vertical t vec
